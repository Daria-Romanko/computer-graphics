...
class FunctionSurface(Polyhedron):
    def __init__(self, func_str, x_range=(-2, 2), y_range=(-2, 2), divisions=10):
        self.func_str = func_str
        self.x_range = x_range
        self.y_range = y_range
        self.divisions = divisions
        
        # Создаем грани поверхности
        faces = self._create_function_surface()
        
        # Вызываем конструктор родительского класса Polyhedron
        super().__init__(faces)
    
    def _create_function_surface(self):
        """Создает поверхность на основе функции (исправленная версия)"""
        faces = []
        vertices_grid = []
        ...
        print(f"Сгенерирована поверхность с {len(faces)} гранями")
        return faces
