...
import math
import sys
from common import Point3D, Face, Polyhedron, Octahedron, Icosahedron, Dodecahedron, AffineTransform, OBJLoader
...
from texture import Texture, TexturedTetrahedron, TexturedCube, TexturedOctahedron, TexturedIcosahedron, TexturedDodecahedron, TextureRenderer
...
class PolyhedronRenderer:
    def __init__(self, width=800, height=600):
        ...
        pygame.display.set_caption("3D Polyhedra Viewer with Texturing")
        self.clock = pygame.time.Clock()
        self.font = pygame.font.Font(None, 36)
        
        self.obj_color = (255, 100, 100)
        ...
        # Обычные многогранники (для режима без текстурирования)
        self.octahedron = Octahedron()
        self.icosahedron = Icosahedron()
        self.dodecahedron = Dodecahedron()
        
        # Текстурированные многогранники
        self.default_texture = Texture()  # текстура по умолчанию
        self.textured_tetrahedron = TexturedTetrahedron(texture=self.default_texture)
        self.textured_cube = TexturedCube(texture=self.default_texture)
        self.textured_octahedron = TexturedOctahedron(texture=self.default_texture)
        self.textured_icosahedron = TexturedIcosahedron(texture=self.default_texture)
        self.textured_dodecahedron = TexturedDodecahedron(texture=self.default_texture)
        
        self.current_polyhedron = self.textured_octahedron
    ...
    def switch_polyhedron(self, polyhedron_type):
        ...
        if self.use_texturing:
            # Используем текстурированные версии
            if polyhedron_type == "octahedron":
                self.current_polyhedron = self.textured_octahedron
                self.current_polyhedron_name = "octahedron"
            elif polyhedron_type == "icosahedron":
                self.current_polyhedron = self.textured_icosahedron
                self.current_polyhedron_name = "icosahedron"
            elif polyhedron_type == "dodecahedron":
                self.current_polyhedron = self.textured_dodecahedron
                self.current_polyhedron_name = "dodecahedron"
            elif polyhedron_type == "tetrahedron":
                self.current_polyhedron = self.textured_tetrahedron
                self.current_polyhedron_name = "tetrahedron"
            elif polyhedron_type == "cube":
                self.current_polyhedron = self.textured_cube
                self.current_polyhedron_name = "cube"
            else:
                return
        else:
            # Используем обычные версии
            if polyhedron_type == "octahedron":
    ...
    def draw_without_z_buffer(self, faces):
        ...
        for face in faces:
            ...
            # Отбрасываем невидимые грани только для перспективной проекции
            if self.projection_type == "perspective" and dot >= 0:
                continue
    ...
    def draw_ui(self):
        ...
        texture_text = "Textured" if self.use_texturing else "Solid"
        
        info_text = f"{self.current_polyhedron_name} | {proj_text} | {mode_text} | {lighting_text} | {texture_text}"
        ...
        for i, line in enumerate(controls):
            ...
            self.screen.blit(text, (10, self.height - 180 + i * 25))
        ...
        for face in faces:
            ...
            if len(points_2d) >= 3:
                ...
                if hasattr(face, 'texture_coords') and face.texture:
                    ...
                    if len(points_2d) == 3:
                        self.lighting.draw_triangle_with_lighting(
                            self.screen, points_2d, depths, 
                            face.vertex_colors if self.lighting.use_lighting else None, 
                            use_zbuffer=True, face_color=face.color
                        )
                    else:
                        ...
                        for i in range(1, len(points_2d) - 1):
                            ...
                            tri_colors = None
                            if self.lighting.use_lighting and face.vertex_colors:
                                tri_colors = [face.vertex_colors[0], face.vertex_colors[i], face.vertex_colors[i + 1]]
                            
                            self.lighting.draw_triangle_with_lighting(
                                self.screen, tri_points, tri_depths, tri_colors, 
                                use_zbuffer=True, face_color=face.color
                            )
        ...
        for event in pygame.event.get():
            ...
            if self.function_mode:
                ...
                if event.key in [pygame.K_1, pygame.K_KP1]:
                    ...
                    self.switch_polyhedron("tetrahedron")
                elif event.key in [pygame.K_4, pygame.K_KP4]:
                    self.switch_polyhedron("cube")
                elif event.key in [pygame.K_5, pygame.K_KP5]:
                    self.switch_polyhedron("dodecahedron")
                elif event.key in [pygame.K_6, pygame.K_KP6]:
                    self.use_texturing = not self.use_texturing
                    print(f"Текстурирование: {'включено' if self.use_texturing else 'выключено'}")
                    self.switch_polyhedron(self.current_polyhedron_name)
    ...
    def draw_arbitrary_line(self):
        ...
        pygame.draw.line(self.screen, (255, 0, 255), p1_2d, p2_2d, 2)
