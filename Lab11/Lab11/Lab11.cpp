...
int main() {
	...
	// ---------------- задание 3 
	// Шейдеры с uniform-переменной для цвета
	const char* vertexShaderSource = R"(
        #version 330 core
        layout (location = 0) in vec3 aPos;
	...
	const char* fragmentShaderSource = R"(
        #version 330 core
        out vec4 FragColor;
        uniform vec4 figureColor;  // Uniform-переменная для цвета
        
        void main() {
            FragColor = figureColor;  // Используем цвет из uniform-переменной
        }
    )";
	// ----------------
	...
	// ---------------- задание 3 
	GLint colorUniform = glGetUniformLocation(shaderProgram, "figureColor"); // получаем location uniform-переменной
	if (colorUniform == -1) {
		cerr << "Ошибка: Не удалось получить location uniform-переменной figureColor!" << endl;
		return -1;
	}
	cout << "Location uniform figureColor: " << colorUniform << endl;
	// ----------------
	...
	while (running) {
		...
		glUniform4f(colorUniform, 1.0f, 0.0f, 0.0f, 1.0f); // передаем цвет в шейдер
